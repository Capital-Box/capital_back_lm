...
export class ExternalProviderFactory {
  ...
  static createExternalProvider(externalProvider: ExternalProviders | null, externalId: string | null): ExternalProvider | null {
    if (!externalProvider) return null;
    if (!externalId) throw new Error('When a provider is provided, an external id must be provided as well');
